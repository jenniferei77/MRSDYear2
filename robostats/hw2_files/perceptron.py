...
def multi_perceptron_predict(x_t, W_t):
    ...
    '''
    find_max = np.dot(W_t,x_t)
    hat_y = np.argmax(find_max)
    return hat_y
...
def multi_perceptron_update(x_t, y_t, W_t):
    ...
    '''
    y_hat = multi_perceptron_predict(y_t, W_t)
    y_diff_yhat = np.expand_dims((np.arange(10) == y_t)*1 - (np.arange(10) == y_hat)*1, axis=1)
    U = np.dot(y_diff_yhat, np.expand_dims(x_t, axis=0))


    # TODO: implement the update rule to compute W_{t+1}
    W_tp1 = W_t + U# replace this line with your implementation
